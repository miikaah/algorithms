...
// (denoting the arrival times), array dep[] (denoting the departure times) and
// the size of the array as inputs and returns the minimum number of platforms required
...
const minPlatforms = ({ input }) => {
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (departure.startsWith("0")) {
            departure = Number(departure.substring(1));
        } else {
            departure = Number(departure);
        }

        let nextArrival;
        let curMinPlatforms = 1;
        for (let j = i + 1; j < length; j++) {
            nextArrival = arrivals[j];

            if (nextArrival.startsWith("0")) {
                nextArrival = Number(nextArrival.substring(1));
            } else {
                nextArrival = Number(nextArrival);
            }

            if (nextArrival > departure) {
                break;
            }
            curMinPlatforms++;
        }
